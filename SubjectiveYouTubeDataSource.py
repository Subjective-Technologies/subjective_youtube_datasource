...
class SubjectiveYouTubeDataSource(SubjectiveDataSource):
    ...
    def __init__(
        ...
        dependency_data_sources: Optional[List[Any]] = None,
        ...
        **kwargs,
        ...
        Args:
            params: Optional params dictionary passed by the datasource launcher
            config: Optional configuration dictionary (overrides params)
        """
        super().__init__(
            name=name,
            session=session,
        ...
        base_config = {}
        try:
            base_config = bb_config.read_config() or {}
        except Exception:
            base_config = {}
        merged_config = {}
        ...
        advanced_options = {}
        if isinstance(self.config.get("advanced_options"), dict):
            advanced_options = self.config["advanced_options"]
        self.whisper_model_size = (
            self.config.get('whisper_model_size')
            or self.config.get('whisper_model')
            or 'base'
        )
        self.max_retries = self.config.get('max_retries') or advanced_options.get('max_retries', 3)
        self.audio_quality = self.config.get('audio_quality') or advanced_options.get('audio_quality', '192')
    ...
    def get_connection_data(self) -> Dict[str, Any]:
        ...
        """
        # Avoid slow network checks in UI paths unless explicitly enabled.
        test_connection = False
        try:
            flag = str(self.config.get("youtube_test_connection", "")).lower()
    ...
    def _execute_external_script(self, script_name: str, input_data: str, form_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(input_data, str) and input_data and not input_data.startswith(("http://", "https://")):
            if not os.path.isabs(input_data):
                candidate_path = os.path.join(script_dir, input_data)
                if os.path.exists(candidate_path):
                    resolved_input = candidate_path

        cmd = [sys.executable, script_path]
        if resolved_input:
            cmd.append(str(resolved_input))

        if os.path.basename(script_path) == "process_youtube_batch.py":
            batch_options = form_data.get('batch_options') or {}
            start_index = batch_options.get('start_index')
            batch_size = batch_options.get('batch_size')
            interactive_mode = batch_options.get('interactive_mode')
        ...
        self._log_info(f"Executing script: {' '.join(cmd)}")
        try:
            ...
            self._log_error(f"Script execution failed: {e}")
            return {
                'success': False,
                'processing_mode': form_data.get('processing_mode'),
        ...
        output_info = stdout if stdout else "Script completed without stdout."
        if len(output_info) > 4000:
            output_info = output_info[:4000] + "...(truncated)"
