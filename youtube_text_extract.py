...
SPANISH_SUMMARIZATION_MODEL = "mrm8488/bert2bert_shared-spanish-finetuned-summarization"  # You can choose a better model if available

# Logging configuration
logging.basicConfig(
    filename='single_video_summary.log',
    level=logging.INFO,
...
def summarize_text(text, summarizer):
    ...
    """
    try:
        max_chunk = 1000  # Adjust based on the model's max token input size
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python youtube_text_extract.py <YouTube_Video_URL>")
        print("Example: python youtube_text_extract.py https://www.youtube.com/watch?v=1234567890A")
        sys.exit(1)
        ...
        if audio_file:
            ...
            if transcript.strip():
                ...
                if lang == "es":
                    ...
                    summarizer = pipeline("summarization", model=SPANISH_SUMMARIZATION_MODEL)
                else:
